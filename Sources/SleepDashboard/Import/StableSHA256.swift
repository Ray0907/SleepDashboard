...
enum StableSHA256 {
	...
	private static func digest(_ input: [UInt8]) -> [UInt8] {
		var message = input
		let bit_count = UInt64(message.count) * 8
		message.append(0x80)
		...
		message.append(contentsOf: withUnsafeBytes(of: bit_count.bigEndian, Array.init))
		var hash = initial_hash

		for offset in stride(from: 0, to: message.count, by: 64) {
			compress(Array(message[offset..<(offset + 64)]), into: &hash)
		}
	...
	private static func compress(_ chunk: [UInt8], into hash: inout [UInt32]) {
		var words = [UInt32](repeating: 0, count: 64)

		for index in 0..<16 {
			let offset = index * 4
			words[index] = UInt32(chunk[offset]) << 24
		...
		for index in 0..<64 {
			let sum_one = rotateRight(e, by: 6) ^ rotateRight(e, by: 11)
				^ rotateRight(e, by: 25)
			let choice = (e & f) ^ ((~e) & g)
			let temporary_one = h &+ sum_one &+ choice &+ constants[index] &+ words[index]
			let sum_zero = rotateRight(a, by: 2) ^ rotateRight(a, by: 13)
				^ rotateRight(a, by: 22)
			let majority = (a & b) ^ (a & c) ^ (b & c)
			let temporary_two = sum_zero &+ majority
	...
	private static let initial_hash: [UInt32] = [
